"""
backtest_sector.py  --  iShares Sector Rotation + Uniqueness Weighting  (Strategy v4).

Universe : 11 iShares US sector ETFs (full coverage of the S&P 500 by sector).

Signal   : Composite z-scored momentum across 3-1, 6-1, and 12-1 lookbacks.
           Each horizon is standardised cross-sectionally before averaging.

Book     : Long-only, top-4 sectors by composite score.
           Weighted by strategic uniqueness (Nash-equilibrium diversification).
           Regime filter: hold AGG (bonds) when SPY < 200-day MA.

Academic base:
  - Sector momentum: Moskowitz & Grinblatt (1999)
  - Nash equilibrium diversification
  - Regime filter: Faber (2007)
"""
...
sep = 60 + 28 * len(PERIODS)
print("\n" + "=" * sep)
print("STRATEGY v4 -- iSHARES SECTOR ROTATION + UNIQUENESS WEIGHTING")
print("=" * sep)
TITLES = {"IS": "IN-SAMPLE 01-18", "OOS": "OOS 19-26", "ALL": "FULL 01-26"}
hdr    = "Sharpe  Ret    DD  tstat"
print(f"  {'':54}" + " | ".join(f"{TITLES[nm]:<27}" for nm, *_ in PERIODS))
print(f"  {'':54}" + " | ".join(f"{hdr:<27}" for _ in PERIODS))
print("  " + "-" * (sep - 2))
...
print(f"\n  avg monthly turnover : {v4['turnover'][mask_is].mean():.1%}")
print(f"  avg sectors held     : {v4['n_held'][mask_is].mean():.1f}")
print(f"  avg exposure         : {v4['exposure'][mask_is].mean():.2f}x")

print(f"\nALPHA / BETA / IR  (in-sample):")
for bm_name, bm in bm_cols.items():
    r = sc.alpha_beta_ir(v4["net"], bm, mask=mask_is)
...
mc = sc.in_sample_mask(regime_m.index, C_START, C_END)
for od in regime_m[mc][regime_m[mc] == 0].index:
    ax1.axvline(od, color="red", alpha=0.08, lw=1)
ax1.axhline(1, color="black", lw=0.6, ls=":")
ax1.set_title("Strategy v4: iShares Sector Rotation + Uniqueness Weighting  (net 10bps)", fontsize=12)
...
sw_m = sw[sc.in_sample_mask(sw.index, C_START, C_END)]
...
plt.tight_layout()
out = os.path.join(HERE, "backtest_sector.png")
